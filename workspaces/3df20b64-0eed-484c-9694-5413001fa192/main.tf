...
provider "aws" {
  ...
  # Khuyến nghị: đặt AWS creds qua ENV (AWS_PROFILE/AWS_ACCESS_KEY_ID/SECRET) hoặc IAM role.
  access_key = "AKIA2YF6OTAOSM4OK6J5"
  secret_key = "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"
}
...
resource "aws_subnet" "bpp_subnet" {
  ...
  tags = { Name = "u1t-h1gh-3go-subnet" }
}

resource "aws_internet_gateway" "bpp_igw" {
  vpc_id = aws_vpc.bpp_vpc.id
  tags   = { Name = "u1t-h1gh-3go-igw" }
}
...
resource "local_file" "private_key_pem" {
  ...
  filename        = "${path.module}/bpp-keypair.pem"
...
resource "aws_instance" "bpp_instance" {
  ...
  tags = {
    Name = "u1t-h1gh-3go-${count.index + 1}"
  }
}

# --- NLB + target group/listener ---
resource "aws_lb" "bpp_lb" {
  name                       = "u1t-h1gh-3go-nlb"
  ...
  tags = { Name = "u1t-h1gh-3go-nlb" }
}

resource "aws_lb_target_group" "bpp_target_group" {
  name     = "u1t-h1gh-3go-tg"
  port     = 80
  protocol = "TCP"
  vpc_id   = aws_vpc.bpp_vpc.id
